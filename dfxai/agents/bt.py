...
def _corner_speed(h: float) -> float:
    ...
    return float(np.sqrt(2.0 * q_need / rho))
...
def _point_at(bb: dict, lead_gain: float = 1.0, g_frac: float = 1.0,
    ...
    if abs(ah) < 1e-3 and abs(av) < 1e-3:
        ...
        mu_des = float(np.arctan2(ah, av))
    bb["a"][0] = np.clip(mu_des / _AC.bank_cmd_limit, -1.0, 1.0)
    bb["a"][1] = np.clip(2.0 * g_frac - 1.0, -1.0, 1.0)
    bb["a"][2] = np.clip(2.0 * throttle - 1.0, -1.0, 1.0)
...
def _horizontal_pursuit(bb: dict, bank_deg: float = 75.0, g: float = 4.0,
    ...
    ah = bb["ata_h"]
    mu = np.sign(ah) * np.deg2rad(bank_deg) if abs(ah) > np.deg2rad(3.0) else 0.0
    bb["a"][0] = float(np.clip(mu / _AC.bank_cmd_limit, -1.0, 1.0))
    bb["a"][1] = float(np.clip(2.0 * (g / _AC.n_max_struct) - 1.0, -1.0, 1.0))
    bb["a"][2] = float(np.clip(2.0 * throttle - 1.0, -1.0, 1.0))
...
# 사전실험 결과(probe3.py 참조) 성능 서열은 v2 > v1 > v3 이고,
    ...
    return Selector("root_v1_horizontal", [
        Sequence("nose_on_hold", [
            Condition("bearing_aligned",
                      lambda bb: abs(bb["ata_h"]) <= np.deg2rad(3.0)),
